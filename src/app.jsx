      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-indigo-500/10 to-purple-500/10">
          ...
          <div className="flex items-center gap-2">
            <button onClick={onDownload} className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg text-sm font-medium hover:shadow-md transition-all">
...
export default function GestionAppareils() {
  ...
  const generateAndDownloadPDF = (type, data) => {
  ...
  const generatePDFFromElement = (container, fileName) => {
...
${companyInfo.email}`
  ...
  const handleDownloadContract = () => {
    if (pdfPreview.data) {
      ...
      showNotification('✓ Téléchargement du contrat en cours...', 'success');
    }
  ...
  const handleDownloadReturnForm = () => {
    if (pdfPreview.data) {
      ...
      showNotification('✓ Téléchargement de la fiche en cours...', 'success');
    }
    setPdfPreview({ isOpen: false, type: '', data: null });
  };
